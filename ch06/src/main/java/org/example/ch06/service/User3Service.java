...
public class User3Service {
    ...
    public User3DTO getUser(String userid){
        ...
        Optional<User3> optUser3 = repository.findById(userid);

        // Entity 존재 여부 확인
        if(optUser3.isPresent()){
            // 존재하면 Optional 타입으로 포장된 Entity 꺼내기
            User3 entity = optUser3.get();
    ...
    public List<User3DTO> getUserAll(){
        ...
        // JPA 조회 메서드 호출, SELECT * FROM User3
        List<User3> entityList = repository.findAll();

        // DTO 리스트 변환
        List<User3DTO> dtoList = entityList.stream()
    ...
    public void remove(String userid){
        // JPA 삭제 메서드 호출, DELETE FROM User3 Where userid='아이디' 수행
